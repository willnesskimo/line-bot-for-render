...
#ngrok是做試驗 <對他下啟動通道的指令，在本機端指定5001，提供給line ever

# 運行應用程式:
# python app.py(這是檔案名稱)
from flask import Flask, request, abort #伺服器的應用模駔，如果程式順利運行，他不會停止
...
from moudles.reply import faq,menu

app = Flask(__name__)#設計伺服器應用程式的模組

configuration = Configuration(access_token='A/QucZdgjKOTzj1CDzwVhh9NrTYBLXh2UtZCCSx2lfeHgxetXKI2ugCl8mc3LFw2RBZJzzAEVEr+Vjh7y5Js5OojC7et5H9fSj4YW8LrKGNpyWYLa7dQoEvvZOWqU25oZIGY6bJab0rWgxStUpAXzQdB04t89/1O/w1cDnyilFU=')
handler = WebhookHandler('cdce99ddfcda3299074713da41644cad')
...
def handle_message(event):
    with ApiClient(configuration) as api_client:
        ...
        user_msg = event.message.text #line制定的指令
        bot_msg = TextMessage(text=f"Hello,你剛才說的是: {user_msg}")

        if user_msg in fag:
            bot_msg=faq[user_msg]

        elif user_msg in ["選單"，"menu","home","主選單"]:
            bot_msg=
...
if __name__ == "__main__":
    ...
    port = int(os.environ.get('PORT', 5001))#ngrok http 5001 開起測試通到
